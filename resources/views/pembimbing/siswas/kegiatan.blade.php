        ...
        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
            <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">
                    {{ $siswa->user->name }}<br>
                    {{ $siswa->kelas->kelas }} - {{ $siswa->jurusan->jurusan }}<br>
                    {{ $siswa->dudi->nama_dudi }}<br>
                </h6>
            </div>
        </div>
        <div class="m-3 mb-2">
        ...
        <div class="card-body px-3 pb-2">
            <div class="table-responsive p-0">
                <table class="table align-items-center mb-0" id="kegiatan">
                    @if (session('success'))
                    <div class="alert alert-success small mx-3">{{ session('success') }}</div>
                    @endif
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <!-- <td class="text-center align-middle text-sm">{{ \Illuminate\Support\Str::words($k->kegiatan, 5, '...') }}</td> -->
                            <td class="text-center align-middle">
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="{{ route('pembimbing.siswa.kegiatan.detail', ['id' => $k->siswa->id, 'id_kegiatan' => $k->id]) }}" style="position: relative; top: 7px;" class="btn btn-info">Detail</a>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-body">
                                        <form action="{{ route('pembimbing.siswa.kegiatan.komentar', $k->id) }}" method="POST">
                                            @csrf
